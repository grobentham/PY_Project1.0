...
1. **Canonical Source Bundle V4** — requires the exact canonical R6 ZIP SHA-256, extracts only frozen Python source and its archive-local dependency closure, rejects unsafe/dynamic/ambiguous source loading, blocks validation/Holdout dependency paths, and permits replacement only of a workspace carrying the exact ownership marker generated by the extractor.
...
6. **Producer Admission Authority V5** — requires canonical-reference replay, candidate replay/parity, and the exact current V4 replay-security contract. It rejects a downgraded replay version, disabled process isolation, changed resource limits, missing worker hash, weakened source-policy guard, or downgraded Source Bundle V4 provenance/safety evidence.
...
Every packaged invocation verifies inherited R6 files, protected R6 files, the exact R7 runtime Python set, launcher and producer operator toolkit hashes before mutable state is used. Runtime integrity also validates the current operator authority semantics, so a stale wrapper cannot pass merely because its stale bytes were re-hashed into a new manifest.

`BUILD_R7_R1.ps1` performs mandatory **Canonical Source Build Preflight V2** against the exact R6 ZIP before it can report PASS. It requires:

- `R7_R1_CANONICAL_SOURCE_BUILD_PREFLIGHT_V2`
- `R7_R1_R6_SOURCE_BUNDLE_V4`
- `R7_R1_R6_SOURCE_PROBE_V2`
- exact canonical-parent SHA-256
- source-only dependency closure and frozen engine-contract proof
- prohibited validation/Holdout source paths blocked
- owned-output-only replacement proof
- a valid ownership-marker SHA-256
- `strategy_executed=false`
- `strategy_retuned=false`
- `final_holdout_accessed=false`
- `producer_admitted=false`

The build records those exact version/safety fields in `R7_R1_BUILD_VERIFICATION.json`. The temporary extracted source stays in the GUID-named build workspace and is removed during cleanup; it is not packaged as a new strategy-source copy.

The builder then compiles, unit-tests and integrity-checks before ZIP creation and repeats verification after clean extraction. Clean extraction revalidates the V2/V4/V2 source-authority versions, prohibited-path proof, owned-output replacement proof, ownership-marker hash and canonical parent identity. The builder intentionally emits a **producer-locked baseline**.
